...
const zhCN = {
  ...
  "shell.shortcutsTaskTerminal": "任务列表：打开终端 / 连接信息",
  ...
  "settings.apiBaseInsecure": "生产环境禁止远程明文 HTTP。请使用 HTTPS，或本机隧道 http://127.0.0.1:.../api",
  "settings.monitorUrlInsecure": "生产环境禁止远程明文监控地址。请使用 HTTPS，或本机隧道 http://127.0.0.1:...",
  "settings.transportInsecureWarning": "当前使用远程明文 HTTP。开发环境仍可用，但密码摘要与 Token 可被重放；生产构建会阻断。请优先改用 HTTPS 或本机隧道。",
  "settings.transportBlocked": "当前 API 地址被安全策略阻断，请求不会发出。请改为 HTTPS 或 http://127.0.0.1 隧道后保存。",
  ...
  "shell.transportBlocked": "API 传输被安全策略阻断，请前往设置改为 HTTPS 或本机隧道。",
  "login.transportBlocked": "当前 API 使用远程明文 HTTP，生产环境已阻断。请先在 API 设置中改为 HTTPS 或本机隧道。",
...
const enUS: Record<TranslationKey, string> = {
  ...
  "shell.shortcutsTaskTerminal": "Tasks: open terminal / connection info",
  ...
  "settings.apiBaseInsecure": "Remote cleartext HTTP is blocked in production. Use HTTPS, or a local tunnel such as http://127.0.0.1:.../api",
  "settings.monitorUrlInsecure": "Remote cleartext monitor URLs are blocked in production. Use HTTPS, or a local tunnel such as http://127.0.0.1:...",
  "settings.transportInsecureWarning": "Remote cleartext HTTP is in use. Allowed in development, but password digests and tokens are replayable. Production builds will block this. Prefer HTTPS or a local tunnel.",
  "settings.transportBlocked": "The current API URL is blocked by the transport policy. Requests will not be sent. Save an HTTPS URL or http://127.0.0.1 tunnel first.",
  ...
  "shell.transportBlocked": "API transport is blocked by the security policy. Open Settings and switch to HTTPS or a local tunnel.",
  "login.transportBlocked": "The current API uses remote cleartext HTTP and is blocked in production. Open API settings and switch to HTTPS or a local tunnel first.",
