...
function toWords(input: Uint8Array) {
  ...
  const wordLength = (((input.length + 8) >>> 6) + 1) * 16;
  const words = new Array<number>(wordLength).fill(0);
  for (let index = 0; index < input.length; index += 1) {
    words[index >> 2] |= input[index] << ((index % 4) * 8);
  }
  words[input.length >> 2] |= 0x80 << ((input.length % 4) * 8);
  words[wordLength - 2] = bitLength & 0xffffffff;
  words[wordLength - 1] = Math.floor(bitLength / 0x100000000);
  return words;
}
...
export function md5ArrayBuffer(buffer: ArrayBuffer) {
  const words = toWords(new Uint8Array(buffer));
  let a = 0x67452301;
  let b = 0xefcdab89;
  let c = 0x98badcfe;
  let d = 0x10325476;
